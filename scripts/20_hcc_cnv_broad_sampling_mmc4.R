# ==============================================================================
# 20_hcc_cnv_broad_sampling_mmc4.R
#
# Purpose
#   Sampling-depth analysis of the six recurrent broad/arm-level CNV events
...
#   - Before sampling, the script reproduces the six published patient-level
...
#     is performed on the same patients with a defined broad-CNV endpoint.
#
...
cat("Script 20: HCC broad CNV sampling from mmc4\n")
cat("================================================================\n\n")
...
EVENT_SET_ID <- "six_recurrent_broad_cnv_events"
...
expected_recovery_from_occupancies <- function(m, n, k) {
  ...
  canonicalize_probability(
    out,
    label = "expected broad-CNV recovery"
  )
}
...
if (DEBUG) {
  fwrite(
    ...
    file.path(
      ext_qc_dir,
      "20_broad_cnv_mmc4_wgs_crosswalk.tsv"
    ),
    sep = "\t"
  )
}
...
for (ii in seq_len(nrow(BROAD_EVENTS))) {
  col <- BROAD_EVENTS$source_column[[ii]]
  direction <- BROAD_EVENTS$direction[[ii]]

  vals <- trimws(as.character(dna[[col]]))

  if (anyNA(vals) || any(!nzchar(vals))) {
    stop(
      "Broad CNV column ", col,
  ...
  if (length(unexpected) > 0L) {
    stop(
      "Unexpected call value(s) in broad column ",
      col, ": ",
  ...
  tab[, `:=`(
    event_id = BROAD_EVENTS$event_id[[ii]],
    source_column = col,
...
if (DEBUG) {
  fwrite(
    ...
    file.path(
      ext_qc_dir,
      "20_broad_cnv_call_domain.tsv"
    ),
    sep = "\t"
  )
}

# -------------------------- long broad call table -----------------------------
long_rows <- vector(
  "list",
  nrow(BROAD_EVENTS)
)
...
broad_long <- rbindlist(
  long_rows,
  use.names = TRUE
)

broad_long[, event_positive :=
...
if (anyNA(broad_long$patient_id) ||
    anyNA(broad_long$n_sectors)) {
  stop(
    "Broad event table failed WGS crosswalk."
  )
}
...
if (anyDuplicated(
  ...
  stop(
    "Duplicate DNA_lib/event_id in broad call table."
  )
}
...
if (nrow(broad_long) !=
    EXPECTED_N_SECTORS *
      nrow(BROAD_EVENTS)) {
  stop(
    "Broad long table is not 490 sectors x 6 events."
  )
}

# ---------------------- full available-sector reference -----------------------
reference <- broad_long[, .(
...
if (nrow(reference) !=
    EXPECTED_N_PATIENTS *
      nrow(BROAD_EVENTS)) {
  stop(
    "Broad reference is not 123 patients x 6 events."
  )
}
...
if (any(reference$full_count < 0L) ||
  ...
  stop(
    "Invalid broad-CNV occupancy relative to patient sector count."
  )
}
...
if (DEBUG) {
  fwrite(
    ...
    file.path(
      ext_dir,
      "20_broad_cnv_full_reference.tsv"
    ),
    sep = "\t"
  )
}
...
trunk_qc[, broad_event_order := match(
  event_id,
  BROAD_EVENTS$event_id
)]
...
trunk_qc[, broad_event_order := NULL]
...
if (anyNA(trunk_qc$published_ratio_match) ||
  ...
  if (DEBUG) {
    fwrite(
      ...
      file.path(
        ext_qc_dir,
        "20_broad_cnv_published_trunk_ratio_qc.tsv"
      ),
      sep = "\t"
    )
  }
  ...
  stop(
    "Failed to reproduce all six published broad-CNV trunk ratios."
  )
}
...
event_reference_summary[, broad_event_order := match(
  event_id,
  BROAD_EVENTS$event_id
)]
...
if (DEBUG) {
  fwrite(
    ...
    file.path(
      ext_dir,
      "20_broad_cnv_event_reference_summary.tsv"
    ),
    sep = "\t"
  )
}
...
patient_expected[
  ...
  expected_detection_recall :=
    canonicalize_probability(
      expected_detection_recall,
      label = "broad expected_detection_recall"
    )
]
...
patient_expected[
  ...
  expected_classification_recall :=
    canonicalize_probability(
      expected_classification_recall,
      label = "broad expected_classification_recall"
    )
]
...
if (nrow(patient_expected) !=
  ...
  stop(
    "Broad patient-expectation table has unexpected dimensions."
  )
}
...
if (any(k1_class != 0)) {
  stop(
    "Broad classification recall at k=1 is not exactly zero."
  )
}
...
if (nrow(valid_pair) > 0L &&
  ...
  stop(
    "Broad classification recall exceeds detection recall."
  )
}
...
if (DEBUG) {
  fwrite(
    ...
    file.path(
      ext_dir,
      "20_broad_cnv_patient_expected_recovery_fixed_n_ge5.tsv"
    ),
    sep = "\t"
  )
}
...
if (DEBUG) {
  fwrite(
    ...
    file.path(
      ext_dir,
      "20_broad_cnv_sampling_operational_heuristic.tsv"
    ),
    sep = "\t"
  )
}
...
provenance <- data.table(
  item = c(
    ...
    "mmc4_patients",
    "mmc4_DNA_libraries",
    "broad_event_definitions",
    "validated_wgs_patients",
    "validated_wgs_sectors",
    "fixed_n_ge5_patients",
    "fixed_patients_with_nonubiquitous_broad_event",
...
if (DEBUG) {
  fwrite(
    ...
    file.path(
      ext_qc_dir,
      "20_broad_cnv_sampling_provenance.tsv"
    ),
    sep = "\t"
  )
}
...
summary_lines <- c(
  "HCC extensions - Script 20 broad CNV sampling from mmc4",
  "================================================================",
  "",
  paste0(
    "mmc4.xlsx MD5: ",
  ...
  "",
  "Published broad-CNV trunk-ratio regression:",
  trunk_lines,
  "",
  sprintf(
    "Six recurrent broad CNV drivers; fixed n>=5 cohort: %d total; %d with >=1 non-ubiquitous broad event",
    nrow(fixed_patients),
    n_info_fixed
  ),
  sprintf(
    "k=4 broad-CNV detection median = %.6f [%.6f, %.6f]",
    k4_detection$median,
    k4_detection$ci_lower,
    k4_detection$ci_upper
  ),
  sprintf(
    "k=4 broad-CNV heterogeneity-classification median = %.6f [%.6f, %.6f]",
    k4_classification$median,
    k4_classification$ci_lower,
    k4_classification$ci_upper
  ),
  sprintf(
    "k=4 paired broad-CNV - protein detection difference = %.6f [%.6f, %.6f] (N=%d)",
    k4_pair$median_broad_minus_protein,
    k4_pair$ci_lower,
    k4_pair$ci_upper,
  ...
  paste0(
    "Guardrail: this analysis represents the six recurrent arm-level CNV ",
    "drivers explicitly called in the published supplementary table; it is ",
    "not claimed to represent every possible arm-level CNV in the genome."
  ),
  paste0(
...
writeLines(
  ...
  file.path(
    ext_dir,
    "20_broad_cnv_sampling_mmc4_summary.txt"
  )
)
